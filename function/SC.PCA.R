...
SC.PCA<-function(pca, groups, filename=NA, dimensions=1:2, legend=FALSE, legend.single=FALSE, 
...
# legend=FALSE	Whether to draw a legend; if TRUE, split the canvas and use the second panel and function SC.PCA.Legend to draw legend
...
#if (legend) cat('Please use information in outputs and function SC.PCA.Legend to draw legend')
if (legend) {
if (legend.single) SC.PCA.Legend(dev.cur(), settings=x)
else SC.PCA.Legend(dev.cur(), settings=unique(x[, 1:4]));
}
...
#################################################################################
# A simplfied version of SC.PCA, which draw PCA of 2 groups of samples
SC.PCA.2Groups<-function(pca, ind1, ind2, label.samples=FALSE, filename=NA, 
...
SC.PCA.Legend(device=dev.cur(), data.frame(names(X), col, pch, cex, l));
}
else  {
i<-c(1, length(ind1)+1);
SC.PCA.Legend(device=dev.cur(), data.frame(groupnames, col[i], pch[i], cex[i]));
}
}

#################################################################################
#################################################################################
SC.AddAPCluster<-function(d, cex=1) {
