CL<-c('black', 'red', 'green3', 'blue', 'cyan', 'magenta', 'orange', 'gray', 'antiquewhite', 'antiquewhite4', 'aquamarine', 'darkgreen', 'darkblue', 'blueviolet', 'brown', 'chartreuse1', 'yellow', 'darkseagreen1', 'deeppink1', 'gold', 'goldenrod4', 'darkmagenta', 'paleturquoise1', 'indianred1', 'hotpink', 'purple4', 'grey39', 'orchid4', 'turquoise4', 'yellow3', 'royalblue4', 'darkorange');


CHOPbic.PCA<-function(pca, groups, filename=NA, dimensions=1:2, legend=FALSE, legend.single=FALSE, 
col=c(), pch=19, cex=2, labels=NA, col.labels='white', propagation=FALSE) {
...
# legend=FALSE	Whether to draw a legend; if TRUE, split the canvas and use the second panel and function CHOPbic.PCA.Legend to draw legend
...
if (propagation) { # Run affinity propagation clustering
x1<-pca$x;
s1<-negDistMat(x1, r=2);
ap<-apcluster(s1);
plot(ap, x1[, 1:2]);
}
else plot(X, Y, col=col, pch=pch, cex=cex, xlim=c(min(X)*1.1, max(X)*1.1), ylim=c(min(Y)*1.1, max(Y)*1.1), 
    xlab=paste('PC', dimensions[1], ', ', per[1], '%', sep=''), ylab=paste('PC', dimensions[2], ', ', per[2], '%', sep=''));
...
#if (legend) cat('Please use information in outputs and function CHOPbic.PCA.Legend to draw legend')
if (legend) {
if (legend.single) CHOPbic.PCA.Legend(dev.cur(), settings=x)
else CHOPbic.PCA.Legend(dev.cur(), settings=unique(x[, 1:4]));
}
else if (!identical(NA, filename))  capture.output(x<-lapply(dev.list(), function(x) dev.off(which=x)));
...
#################################################################################
# A simplfied version of CHOPbic.PCA, which draw PCA of 2 groups of samples
CHOPbic.PCA.2Groups<-function(pca, ind1, ind2, label.samples=FALSE, filename=NA, 
    groupnames=c('A', 'B'), dimensions=1:2, col=c(4, 2), pch=c(19, 19), cex=c(2,2), propagation=FALSE) {
# pca			Outputs of prcomp()
...
CHOPbic.PCA.Legend(device=dev.cur(), data.frame(names(X), col, pch, cex, l));
}
else  {
i<-c(1, length(ind1)+1);
CHOPbic.PCA.Legend(device=dev.cur(), data.frame(groupnames, col[i], pch[i], cex[i]));
}
}
...
CHOPbic.PCA.Legend<-function(device, settings, labels=as.vector(settings[,1])) {
