...
SC.Excel<-function(values, fileName=paste(Sys.Date(), 'R2Excel', sep='_'), sheetNames=names(values), type='xlsx', settings=rep(list(), length(values))) {
# values		a list of data.frame or matrix
# fileName		the prefix of output file
# sheetNames	the names of individual worksheets
# settings		a list of extra formatting parameters used by SC.ExcelSheet, each element of the list corresponds to a worksheet
...
for (i in 1:length(values)) {
	do.call(SC.ExcelSheet, append(list(wb=wb, values=values[[i]], name=as.character(sheetNames[i])), settings[[i]])); 
...
SC.ExcelSheet<-function(wb, values, name, col.names=TRUE, row.names=TRUE, id.name='ID',
