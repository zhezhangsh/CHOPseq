# This is a modified version of GSEA.1.0.R function written by the Broad Institute
CHOPbic.GSEA<-function(
    d, 
    gene.desc=rownames(d), 
    ind.g1=1:round(ncol(d)/2), 
    ind.g2=(max(ind.g1)+1):ncol(d), 
    groupnames=c('A', 'B'), 
    gmt='', 
    gs.size.cutoff=c(15, 500), 
    nperm=1000,
    unlog=TRUE
    ) {
d<-d[, c(ind.g1, ind.g2)];
if (unlog) d<-exp(d*log(2));

# create GCT file
gct<-'expr.gct';
write.table('#1.2', gct, row=FALSE, col=FALSE, quote=FALSE, sep='\t');
write.table(paste(nrow(d), ncol(d), sep='\t'), gct, append=TRUE, row=FALSE, col=FALSE, quote=FALSE);
x<-data.frame(NAME=rownames(d), Description=gene.desc, d);
write.table(paste(names(x), collapse='\t'), gct, append=TRUE, row=FALSE, col=FALSE, quote=FALSE, sep='\t');
write.table(x, gct, append=TRUE, row=FALSE, col=FALSE, quote=FALSE, sep='\t');

# create CLS file
cls<-'sample.cls';
write.table(paste(ncol(d), 2, 1), cls, row=FALSE, col=FALSE, quote=FALSE, sep='\t');
write.table(paste("#", groupnames[1], groupnames[2]), cls, append=TRUE, row=FALSE, col=FALSE, quote=FALSE, sep='\t');
write.table(paste(rep(groupnames[1:2], c(length(ind.g1), length(ind.g2))), collapse=' '), cls, append=TRUE, row=FALSE, col=FALSE, quote=FALSE, sep='\t');

results<-GSEA(input.ds=gct, input.cls=cls, gs.db=gmt, output.directory=paste(getwd(), '/', sep=''), 
    doc.string = paste(groupnames[1], groupnames[2], sep='_vs_'), reshuffling.type = "sample.labels", 
	nperm = nperm, gs.size.threshold.min = gs.size.cutoff[1], gs.size.threshold.max = gs.size.cutoff[2], 
    reverse.sign=TRUE, save.intermediate.results = TRUE)

results<-rbind(results[[1]], results[[2]]);
results[[2]]<-as.numeric(as.vector(results[[2]]));
for (i in 4:ncol(results)) results[[i]]<-as.numeric(as.vector(results[[i]]));
results<-results[order(results[, 'NES']),];
...
l<-length(x[x]); 
       ...
       if (non.interactive.run == F) {
           if (.Platform$OS.type == "windows") {
               gs.filename <- "All_Plots";
               windows(width = 18, height = 6*min(4,l))
           } else if (.Platform$OS.type == "unix") {
               gs.filename <- "All_Plots.pdf";
               pdf(file=gs.filename, height = 6*min(4, l), width = 18)
           }
       } else {
           if (.Platform$OS.type == "unix") {
              gs.filename <- "All_Plots.pdf";
...
if (l>0) nf <- layout(matrix(1:(3*min(4, l)), min(4, l), 3, byrow=T), 1, c(1, 1, 1), TRUE)
#par(mfrow=c(min(5,l), 3));
...
#       print(gene.report)
            ...
            plot(ind, Obs.RES[i,], main = main.string, sub = sub.string, xlab = "Gene List Index", ylab = "Running Enrichment Score (RES)", xaxt='n', xlim=rev(c(1, N)), ylim=c(min.plot, max.plot), type = "l", lwd = 2, cex = 1, col = col)
            at<-axis(1, labels=FALSE, tick=FALSE);
            axis(1, at=N-at, labels=at);
            for (j in seq(1, N, 20)) {
